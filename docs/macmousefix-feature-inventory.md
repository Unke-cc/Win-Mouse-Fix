...
> [!NOTE]
> 本文是历史参考资料，用于记录早期产品调研，不代表 Win Mouse Fix `0.1.3 Beta` 的当前功能、界面或实现方式。当前版本以 [应用设计文档](./application-design.md)、[技术选择](./technology-selection.md) 和发布检查清单为准。
...
| Scrolling | 平滑程度、触控板模拟、方向反转、速度、精确滚动和组合键模式 | Win Mouse Fix 当前实现基础滚轮设置、组合修饰键和触摸板高频输入保护；不等同于触摸板设备级识别 |
...
- 拖动与滚轮触发：按住鼠标键并移动或滚动，用于连续滚动、窗口/桌面切换等手势；Win Mouse Fix 当前只提供预定义的融合动作。
...
Scrolling 包含平滑等级、触控板式滚动、反向、速度、慢速滚轮精确控制，以及横向、缩放、快速、精确四类组合键模式。Win Mouse Fix `0.1.3 Beta` 当前支持反向、速度、平滑度，以及由 Ctrl、Alt、Shift、Win 组成的组合键；触摸板保护根据连续输入特征判断，不是设备来源识别。
...
Win Mouse Fix `0.1.3 Beta` 已包含完整动作列表、融合动作、滚轮组合键、配置管理、应用排除、全屏暂停、双来源更新检查和安装程序。发布前仍需验证普通窗口、全屏应用、触摸板连续滑动、实体鼠标滚轮、配置保存和打包后的独立运行。
