...
GitHub 和 Gitee 应使用相同的 `v0.1.3-beta` tag，并上传同一份安装程序。正式版不能标记为预发布；Beta 版必须标记为预发布。`v0.1.3-beta.2` 高于 `v0.1.3-beta.1`，同版本正式版高于 Beta。发现更高版本后，应用会显示来源并询问用户是否打开对应 Release 页面。
